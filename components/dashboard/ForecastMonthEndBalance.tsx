...
export default function ForecastMonthEndBalance({
  ...
  const forecastCurrencies = model.currencies.filter(
    (currency) => currency.forecast.availability === "available"
  );
  const hasHistoricalUnavailable = model.currencies.some(
    (currency) => currency.forecast.availability === "historical-unavailable"
  );
  const visibleEntries = forecastCurrencies.slice(0, 3);
  const scheduledOccurrencesCount = forecastCurrencies.reduce(
    (sum, currency) => sum + currency.forecast.scheduledOccurrencesCount,
    0
  );
  const activeScheduledRuleCount = forecastCurrencies.reduce(
    (sum, currency) => sum + currency.forecast.scheduledRuleCount,
    0
  );
  ...
  return (
    <div>
      ...
      <div className="mt-5 gl-inner-card rounded-2xl p-3">
        ...
        <p className="mt-2 text-[11px] leading-5 text-gray-400">{confidenceHelper(confidence)}</p>
      </div>

      {hasHistoricalUnavailable ? (
        ...
        <div className="mt-3 space-y-3">
          {visibleEntries.map((currency) => {
            const forecast = currency.forecast;
      ...
      {!hasHistoricalUnavailable && forecastCurrencies.length > visibleEntries.length && (
        <div className="mt-3 text-[11px] text-gray-500">+{forecastCurrencies.length - visibleEntries.length} more currency balance(s)</div>
      )}
    </div>
  );
}
