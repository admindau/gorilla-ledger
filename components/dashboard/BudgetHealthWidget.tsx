...
function statusForRatio(ratio: number, riskThreshold: number) {
  if (ratio > 1) return { label: "Over", className: "border-white/70 text-white", helper: "Budget exceeded" };
  if (ratio > riskThreshold) return { label: "At risk", className: "border-gray-500 text-gray-200", helper: "Approaching limit" };
  return { label: "Healthy", className: "border-gray-700 text-gray-300", helper: "On track" };
}

export default function BudgetHealthWidget({ model, summaries, riskThreshold }: BudgetHealthWidgetProps) {
  ...
  const highlighted = summaries.slice().sort((a, b) => b.usedRatio - a.usedRatio).slice(0, 4);

  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div><h3 className="text-sm font-semibold tracking-tight">Budget Health</h3><p className="mt-1 text-[11px] text-gray-400">Visual budget status for {monthLabel}.</p></div>
        <div className="flex flex-wrap gap-2 text-[11px] text-gray-300">
          <span className="rounded-full border border-gray-800 px-2 py-1">{budgetsOnTrack} healthy</span>
          <span className="rounded-full border border-gray-800 px-2 py-1">{budgetsAtRisk} at risk</span>
          <span className="rounded-full border border-gray-800 px-2 py-1">{budgetsOver} over</span>
        </div>
      </div>

      {totalBudgets === 0 ? (
        <div className="mt-5 gl-empty-state rounded-2xl p-4 text-sm text-gray-400">No budgets set for this month yet. Add budgets to monitor financial limits.</div>
      ) : (
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {highlighted.map((item) => {
            const isCurrencySafe = item.isScorable;
            const status = isCurrencySafe ? statusForRatio(item.usedRatio, riskThreshold) : { label: "Unscored", className: "border-gray-700 text-gray-400", helper: item.scoringReason ?? "This budget cannot be scored safely" };
