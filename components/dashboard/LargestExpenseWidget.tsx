    ...
    <div>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[11px] text-gray-400">
            Biggest single expense per currency in {monthLabel}.
          </p>
        </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
          {items.map((item) => (
            <div key={item.id} className="gl-inner-card rounded-2xl p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="truncate text-[11px] uppercase tracking-wide text-gray-500">
                  {item.categoryName}
                </div>
                <span className="rounded-full border border-white/10 bg-white/[0.035] px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-gray-300">
                  {item.currencyCode}
                </span>
              </div>

              <div className="mt-3 text-2xl font-semibold leading-none tracking-tight tabular-nums text-white">
