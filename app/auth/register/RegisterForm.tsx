...
export function RegisterForm({ next, familyInvite = false }: { next: string; familyInvite?: boolean }) {
  ...
  return <div className="gl-auth-card gl-card w-full max-w-md">
    ...
    {successMsg && <div className="gl-auth-alert gl-auth-alert-success" role="status"><p>{successMsg}</p><p className="mt-2 text-xs leading-5 text-white/65">Look in Focused, Other, and Junk, or search for no-reply@savvyrilla.tech. If you requested more than one email, open only the newest link.</p></div>}
