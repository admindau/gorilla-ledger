    ...
    async function boot() {
      ...
      try {
        if (inviteToken) {
          ...
          if (!accepted.ok) throw new Error(apiError(body, "Unable to accept this invitation."));
          if (!cancelled) setSuccess("You’ve joined the household ledger.");
  ...
  return <PageShell className="gl-page-stack" size="lg">
    ...
    {loading || !overview ? <Card><CardBody><p className="text-sm text-gray-400">Loading household access…</p></CardBody></Card> : <>
      ...
      <Card variant="premium"><CardHeader><div><p className="gl-page-eyebrow">Current household</p><h2 className="text-xl font-semibold text-white">{overview.ledger.name}</h2></div><Badge variant={isOwner ? "success" : "neutral"}>{isOwner ? "Owner" : "Editor"}</Badge></CardHeader><CardBody className="space-y-4">
        ...
        <div className="divide-y divide-white/10 rounded-xl border border-white/10">{overview.members.map((member) => <div key={member.user_id} className="flex items-center justify-between gap-4 p-4"><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{member.email}</p><p className="mt-1 text-xs text-gray-500">{member.role === "owner" ? "Ledger owner" : "Can view and edit"}{member.is_current_user ? " · You" : ""}</p></div><div className="flex items-center gap-2"><Badge variant={member.role === "owner" ? "success" : "neutral"}>{member.role}</Badge>{isOwner && member.role !== "owner" ? <Button variant="danger" size="sm" disabled={busy} onClick={() => void removeMember(member)}>Remove</Button> : null}</div></div>)}</div>
      </CardBody></Card>
