  ...
  return <PageShell className="gl-page-stack" size="lg">
    ...
    <>
      ...
      <Card variant="premium"><CardHeader><div><p className="gl-page-eyebrow">Current household</p><h2 className="text-xl font-semibold text-white">{overview.ledger.name}</h2></div><Badge variant={isOwner ? "success" : "neutral"}>{isOwner ? "Ledger owner" : overview.ledger.role === "editor" ? "Editor" : "Viewer"}</Badge></CardHeader><CardBody className="space-y-4">
        ...
        <div className="divide-y divide-white/10 rounded-xl border border-white/10">{overview.members.map((member) => <div key={member.user_id} className="grid min-w-0 gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="break-all text-sm font-medium text-white">{member.email}</p><p className="mt-1 text-xs text-gray-400">{member.role === "owner" ? "Ledger owner" : member.role === "editor" ? "Can view and edit" : "View only"}{member.is_current_user ? " · You" : ""}</p></div><div className="flex flex-wrap items-center gap-2"><Badge variant={member.role === "owner" ? "success" : "neutral"}>{member.role === "owner" ? "Owner" : member.role === "editor" ? "Editor" : "Viewer"}</Badge>{isOwner && member.role !== "owner" ? <Button variant="danger" size="sm" disabled={busy} onClick={() => void removeMember(member)}>Remove</Button> : null}</div></div>)}</div>
      </CardBody></Card>
      {isOwner ? <Card><CardHeader><div><p className="gl-page-eyebrow">Add someone</p><h2 className="text-lg font-semibold text-white">Invite a family member</h2></div></CardHeader><CardBody className="space-y-5">
        ...
        {overview.invitations.length ? <div><h3 className="mb-2 text-sm font-semibold text-white">Pending invitations</h3><div className="divide-y divide-white/10 rounded-xl border border-white/10">{overview.invitations.map((invitation) => <div key={invitation.id} className="grid min-w-0 gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="break-all text-sm text-white">{invitation.email}</p><p className="mt-1 text-xs text-gray-400">{invitation.role === "viewer" ? "Viewer" : "Editor"} · Expires {new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(invitation.expires_at))}</p></div><Button variant="ghost" size="sm" disabled={busy} onClick={() => void revokeInvitation(invitation)}>Revoke</Button></div>)}</div></div> : null}
